...
class TaskControllerTest {
    ...
    void setUp() {
        ...
        // Provide default behavior for the mock
        when(mockTask.getTask_Id()).thenReturn(100L);
        when(mockTask.getTask_Title()).thenReturn("Mock Task Title");
        when(mockTask.getDescription()).thenReturn("Mock Description");
        when(mockTask.getAssigned_To()).thenReturn("55L");
        when(mockTask.getStatus()).thenReturn("In Progress");
        when(mockTask.getDeadline()).thenReturn(LocalDateTime.now());
    }
    ...
    void testGetMyTasks() {
        // Arrange
        // Mock the request + the token
        HttpServletRequest mockRequest = mock(HttpServletRequest.class);
        when(mockRequest.getHeader("Authorization")).thenReturn("Bearer ABCDE123456");
        
        // Mock the JWT secret used in TaskController
        when(jwtProperties.getSecret()).thenReturn("MyTestSecret12345678901234567890");

        // For the 'subject' in the token, we just manually parse it in the controller with the real code:
        // We'll bypass real parsing by giving a minimal valid token structure that includes the subject
        // But here, we can either do real token creation or short-circuit the test if the user has trouble
        // with real Jwts parsing. For a simpler approach, we can skip actually verifying the token's validity:
        
        // If you get actual parse issues, you can do something like:
        ...
        // But let's assume the user is using the standard parse. We'll do it fully:
